require_relative 'send-mail2.rb'
...
while true
    ...
    if response.match(/data-hasavailability="1"/)
        ...
        subject = "BeOurGuest #{reservation_date}"
        send_mail(notify_email, subject, available_times)
        break
    else
        puts "#{iterations}. not available...sleeping for #{loop_sleep_seconds}"
        iterations += 1
